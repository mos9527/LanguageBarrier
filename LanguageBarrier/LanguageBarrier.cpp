		...
		if (!IsInitialised) {
			if (sigScan("game", "canaryMagesLauncher") != NULL) {
				...
				if (config["patch"].count("hijackLauncher") == 1) {
					std::string cmdl = config["patch"]["hijackLauncher"].get<std::string>();
					size_t pos = cmdl.find_first_of(" ");
					std::string executable = cmdl;
					...
					ss << "hijackLauncher: spawning " << cmdl;
					LanguageBarrierLog(ss.str());

					if (!CreateProcessA(
						executable.c_str(),
						const_cast<char*>(cmdl.c_str()),
					...
					{
						std::stringstream ss;
						ss << "hijackLauncher: CreateProcess failed. LastError=" << GetLastError();
						MessageBoxA(NULL, ss.str().c_str(), "LanguageBarrier", MB_OK | MB_ICONERROR);
						exit(1);
					}
